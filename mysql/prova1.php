...
if (!isset($_SESSION["connection"])) $_SESSION["connection"] = new Mysqli();
    ...
    <div class="text-center">
        ...
        <table class='table table-striped'>
            <tr>
                ...
                <th>
                    <a href="prova1.php?<?php if ($_SESSION["ordre"] == " order by id_edit" && $_SESSION["as"] == " asc") echo "dir=desc";
                    else if ($_SESSION["ordre"] == " order by id_edit" && $_SESSION["as"] == " desc") echo "dir=asc";
                    else echo "ordre=id_edit&dir=asc";

                    ?>">ID</th>
                <th>
                    <a href="prova1.php?<?php if ($_SESSION["ordre"] == " order by nom_edit" && $_SESSION["as"] == " asc") echo "dir=desc";
            ...
            $cursor = $_SESSION["connection"]->query($sql) or die("fallo garrafal");

            $cursor2 = $_SESSION["connection"]->query($count) or die("fallo garrafal count");

            $_SESSION["tuples"] = ($cursor2->fetch_all())[0][0];


            while ($row = $cursor->fetch_assoc()) {
                echo '<tr><td><a href="editar.php?id='.$row["id_edit"].'" class="btn btn-success">Editar</a></td><td class="align-middle">' . $row["id_edit"] . '</td><td class="align-middle">' . $row["nom_edit"] . '</td></tr>';
            }
            echo "</table>";
            $a = paginacio();
            ?>
            ...
            <a href="afegir.php" class="btn btn-primary mb-4 mt-2">Afegir Editor</a>
    </div>
</div>
</body>